<template>
  ...
  <div v-if="show.show" class="show" @click.self="showWhat('')">
    <div class="leftImg">
...
function setWalls(values) {
  ...
  const ffi: any = require('ffi-napi')
  console.log(ffi,'ffi');
  
  //   let values:any = document.getElementById('iptss');
  const myDlls = ffi.Library('E:\\outCode\\WallPaper-electron\\libwalls.dll', {
    setWallpaper: ['void', ['string']]
  })
  myDlls.setWallpaper(gbk.convert(values, 'GBK'))
}
// 存放api获取的分类壁纸
const paperCat: any = reactive({
  data: ''
})
//存放分类名称
const paperName: any = reactive({
  data: ''
})
// 初始化数据,获取分类
async function getCategory() {
  try {
    await axios
      .get('http://wp.birdpaper.com.cn/intf/getCategory')
      .then((res) => {
        // console.log(res);
        paperName.data = res.data.data
      })
      .then(() => {
        const inits: any = document.getElementById('init2')
...
function setStyle(event) {
  const all = event.currentTarget.parentElement.children
...
function getAdd(fileURL) {
  ...
  const path = 'D:\\pictures\\wallpaper\\'
  if (!fs.existsSync(path)) {
    fs.mkdirSync(path)
  }
  console.log(1333)
  const fileName = path + fileURL.substring(fileURL.length - 10)
  if (fs.existsSync(fileName)) {
    //判断文件是否存在，如果存在则直接设置
    setWalls(fileName)
    alert('设置成功！')
  } else {
    //如果不存在，则先下载图片，然后回调函数设置壁纸
    request(fileURL)
...
.show {
  ...
  background: rgba(rgb(205, 197, 197), 0.7);
  display: flex;
  ...
  .leftImg {
    ...
    height: 80%;
    margin: 5px;
    background: rgba(226, 235, 200, 0.7);
    float: left;
    border-radius: 20px;
  }
  .rightMsg {

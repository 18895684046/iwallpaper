<template>
  <div class="dynamic">
    <div class="select">
      ...
      <input id="setWapper" type="file" accept="video/*" @change="sets()" />
    </div>
  </div>
</template>
...
function sets(): void {
  ...
  const values: any = document.getElementById('setWapper')
  console.log(values.files[0].path)
  setWappers(gbk.convert(values.files[0].path, 'GBK'))
}
function setWappers(paths): void {
  ...
  const W32 = new ffi.Library('user32', {
    ...
    // 将指定的消息发送到一个或多个窗口
    SendMessageTimeoutW: ['int32', ['int32', 'int32', 'int32', 'int32', 'int32', 'int32', 'int32']],
  ...
  function setDynamicWallpaper(ffplayw) {
    ...
    W32.EnumWindows(
      ffi.Callback('bool', ['int32', 'int32'], (tophandle) => {
        ...
        const SHELLDLL_DefView = W32.FindWindowExW(tophandle, 0, TEXT('SHELLDLL_DefView'), 0)
